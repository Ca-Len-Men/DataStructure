...
/* Map is a B+ Tree.
You must have copy constructor for keytype and valuetype.
...
#pragma region Sub Class
		...
		public:
			...
			void push_value(pairtype*& pointer, size_t index) {
				if (index == 0u) {
					if (values[1] == nullptr)	// 1 0
						values[1] = values[0];
					else {	// 1 2 0
						pairtype* temp(pointer);
						pointer = values[1];
						values[1] = values[0];
						values[0] = temp;
						return;
			...
			// Find branch
			size_t find_branch(const keytype& value) {	// Node is not empty
				if (value < values[0]->key)
					return 0u;
				if (values[1] == nullptr || value < values[1]->key)
					return 1u;
				return 2u;
			}

			// Index of min value from this leaf node to next leaf node
			pairtype* min_to_right() {
				if (values[0])
					return values[0];
...
#pragma region Private Methods
	// Default constructor
		...
		void viewRecur(node* rootnode, size_t layer = 0u) const
		{
			...
			for (int i = count; i >= 0; --i)
			{
				...
				if (i < count) {
					...
					if (i == count - 1)
						putchar('[');
					printf("%8d", rootnode->values[i]->value);
					if (i == 0u)
						putchar(']');
					line;
				}
		...
		// Push recursion
		bool push_recursion(node*& rootnode, const pairtype& value, pairtype*& ptr_value, node*& ptr_node) {
			size_t index = rootnode->find_branch(value.key);
			if (rootnode->num < 0) {	// rootnode is leaf
				if (index > 0 && value.key == rootnode->values[index - 1u]->key)		// Duplicated
					return false;

				// Don't duplicate
				ptr_value = new pairtype(value);
		...
		// Pop recursion
		bool pop_recursion(node*& rootnode, const keytype& key, pairtype**& ref_value) {
			size_t index = rootnode->find_branch(key);
			// Rootnode is leaf
			if (rootnode->num < 0) {
				if (index == 0u)	return false;	// Does not contains value
				--index;
				if (key == rootnode->values[index]->key)	// Have contains value
			...
			if (index > 0u && key == rootnode->values[index - 1u]->key)
				ref_value = rootnode->values + index - 1u;
			bool get = pop_recursion(rootnode->ptrs[index], key, ref_value);
		...
		pairtype* copy_recursion(node*& rootnode, node* othernode, queue<node*>& qu) {
			rootnode = new node;
			rootnode->num = othernode->num;

			// Node is leaf
			if (othernode->num < 0) {
				rootnode->values[0] = new pairtype(*othernode->values[0]);
				if (othernode->values[1])
					rootnode->values[1] = new pairtype(*othernode->values[1]);
				qu.push(rootnode);
				return rootnode->values[0];
			}

			pairtype* min_value_of_branch = copy_recursion(rootnode->ptrs[0], othernode->ptrs[0], qu);
...
#pragma region Constructors and Destructor
		map() : root(nullptr), size(0u)
		{
			default_constructor();
		}

		map(std::initializer_list<pairtype> list) : root(nullptr), size(0u)
		{
			default_constructor();
		...
		map(anyIterator begin, anyIterator end) : root(nullptr), size(0u)
		{
			default_constructor();
			for (; begin != end; ++begin)
				push(*begin);
		...
		map(const map& source) : root(nullptr), size(source.size)
		{
			default_constructor();
			if (source.empty())
				return;
...
#pragma region Methods
	// Push an item into Map
		bool push(const keytype& key, const valuetype& value) {
			return push(pairtype(key, value));
		}

		// Push an item into Map
		bool push(const pairtype& value) {
			if (root == nullptr) {
				root = new node;
				root->values[0] = new pairtype(value);
				root->num = -1;
		...
		bool contains(const keytype& key) const {
			...
			node* res = root;
			while (res->num > 0)
				res = res->ptrs[res->find_branch(key)];
			size_t index = res->find_branch(key);

			if (index == 0u)
				return false;
			return key == res->values[index - 1u]->key;
		}
...
#pragma region Operators
	// Map is not empty
		...
		// True if Map has contains the value
		valuetype& operator[](const keytype& key) const {
			node* res = root;
			while (res->num > 0)
				res = res->ptrs[res->find_branch(key)];
			return res->values[res->find_branch(key) - 1u]->value;
		}

		// Get item at index
		const pairtype& operator()(int index) const {
			node* res = root;
...
#pragma region Iterator
		class iterator {
			...
			iterator(const map* _ref, const node* _ptrNode, size_t _idx) :
...
#pragma region Const Iterator
		class const_iterator {
			...
			const_iterator(const map* _ref, const node* _ptrNode, size_t _idx) :
				ref(_ref), ptrNode(_ptrNode), idx(_idx) {}
		public:
			//=====     Constructors     =====//
...
#pragma endregion
	...
	template <class keytype, class valuetype>
	std::ostream& operator<<(std::ostream& os, const map<keytype, valuetype>& set) {
		set.out();
		return os;
	}
}
