#ifndef _FISH_HASHSET_
#define _FISH_HASHSET_
...
Hash Set : hashset<datatype>						=
===================================================*/

/* Hash Set is a dynamic array and Singly Linked List.
You must have copy constructor, operator%(int), operator= for your datatype.*/
...
namespace dsa
{
	...
	#pragma region Sub Class
			...
			public:
				datatype info;
				node* next;
				node(const datatype& value) : info(value), next(nullptr) {}
			};
	#pragma endregion
	...
	#pragma region Private Methods
		...
		int hash_function(const datatype& value) const {
			int mod = value % this->capacity;
			if (mod < 0)
				mod = -mod;
			return mod;
		}
		...
		void resize(bool expand = true) {
			...
			for (i = 0u; i < this->size; ++i) {
				tail = head;
				head = head->next;
				index = this->hash_function(tail->info);
	...
	#pragma region Constructors and Destructor
			...
			hashset(std::initializer_list<datatype> list) : size(0u), capacity(1u)
			{
				this->ptr = new node * [1u];
				this->ptr[0] = nullptr;
				for (const datatype& i : list)
					this->push(i);
			}

			template <class anyIterator>
			hashset(anyIterator begin, anyIterator end) : size(0u), capacity(1u)
			{
				this->ptr = new node * [1u];
				this->ptr[0] = nullptr;
				for (; begin != end; ++begin)
			...
			hashset(const hashset& source) : size(0u), capacity(1u)
			{
				this->ptr = new node * [1u];
				this->ptr[0] = nullptr;
			...
			hashset(hashset&& source) : size(source.size), capacity(source.capacity), ptr(source.ptr)
			{
				source.ptr = nullptr;
			}
	...
	#pragma region Operators
		// Hash Set contains the value
			...
			// Copy data from other Hash Set
			hashset& operator=(hashset&& source) noexcept {
				this->delete_function();
				this->ptr = source.ptr;
	...
	#pragma region Iterator
			class iterator {
				...
				iterator(hashset* _ref, node* _ptr, int _index) : ref(_ref), ptr(_ptr), index(_index) {}
			public:
				...
				// Access value in Hash Set
				const datatype& operator*() const {
					return this->ptr->info;
				}

				// Access index of value in Hash Set
				const datatype* operator->() const {
					return &this->ptr->info;
				}
	...
	#pragma region Const Iterator
			class const_iterator {
				...
				const_iterator(const hashset* _ref, const node* _ptr, int _index) : ref(_ref), ptr(_ptr), index(_index) {}
			public:
	...
	template <class datatype>
	std::ostream& operator<<(std::ostream& os, const hashset<datatype>& source) {
		source.out();
		return os;
	}
}
